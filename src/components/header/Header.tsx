...
// Простые компоненты-заглушки для демонстрации
const Button = ({
  type = 'primary',
  size = 'medium',
  onClick,
  ...
  <button
    className={`${styles.button} ${styles[`button_type_${type}`]} ${styles[`button_size_${size}`]} ${className}`}
    onClick={onClick}
  >
    {children}
  </button>
...
const Icon = ({ name, size = 24, className = '', fill }: any) => {
  if (name === 'arrow-down-icon') {
    return (
      <svg
        width={size}
        height={size}
  ...
  if (name === 'search-icon') {
    return (
      <svg
        ...
        className={`${styles.icon} ${className}`}
      >
        <path d="M11.5349 21.0698C6.27908 21.0698 2 16.7907 2 11.5349C2 6.27908 6.27908 2 11.5349 2C16.7907 2 21.0698 6.27908 21.0698 11.5349C21.0698 16.7907 16.7907 21.0698 11.5349 21.0698ZM11.5349 3.39535C7.04187 3.39535 3.39535 7.05118 3.39535 11.5349C3.39535 16.0186 7.04187 19.6745 11.5349 19.6745C16.0279 19.6745 19.6745 16.0186 19.6745 11.5349C19.6745 7.05118 16.0279 3.39535 11.5349 3.39535Z"/>
        <path d="M21.3019 21.9996C21.1252 21.9996 20.9484 21.9345 20.8089 21.7949L18.9484 19.9345C18.6787 19.6647 18.6787 19.2182 18.9484 18.9484C19.2182 18.6787 19.6647 18.6787 19.9345 18.9484L21.7949 20.8089C22.0647 21.0787 22.0647 21.5252 21.7949 21.7949C21.6554 21.9345 21.4787 21.9996 21.3019 21.9996Z"/>
      </svg>
    );
  }
...
const Input = ({
  ...
  <div className={`${styles.input__container} ${className || ''}`}>
    <div className={styles.input__wrapper}>
      {icon && (
        <div
          className={`${styles.input__icon_container} ${iconPosition === 'left' ? styles.input__icon_left : styles.input__icon_right}`}
        >
          {icon}
        </div>
      )}
...
const Logo = ({ size = 160, onClick, className = '' }: any) => {
...
const setQuery = (value: string) => ({ type: 'SET_QUERY', payload: value });
  ...
  return (
    ...
    >
      ...
      <div className={styles['header__links']}>
        ...
        {/* Кнопка «Все навыки» с функциональностью клика */}
        <div
      ...
      <div className={styles['header__theme-toggle']}>
        <Icon name='moon-icon' size={24} />
      </div>

      {/* Блок авторизации: кнопки «Войти/Зарегистрироваться» */}
